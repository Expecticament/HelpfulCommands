...
public class CMD_extinguish implements IHelpfulCommandsCommand {
        ...
        for(Entity i : targets) {
            if(extinguish(i)) {
                ...
                if(commandFeedback && i.isPlayer() && i != player) {
                    ((ServerPlayerEntity) i).sendMessage(Text.translatable("commands.extinguish.success.self").setStyle(HelpfulCommands.style.tertiary));
                }
            }
        }
    ...
    private static boolean extinguish(Entity entity) {
        if(!entity.isOnFire()) {
            return false;
        }
