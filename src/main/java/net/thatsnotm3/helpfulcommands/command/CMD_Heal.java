...
public class CMD_Heal{
    ...
    static final String cmdName="heal";
    ...
    public static int run(CommandContext<ServerCommandSource> ctx, Collection<ServerPlayerEntity> targets) throws CommandSyntaxException{
        ...
        if(targets!=null){
            ...
            if(i>0){
                ...
                ;
                MutableText msg=Text.translatable("message.command.heal.target",Text.literal(Integer.toString(i)).setStyle(playerList)).formatted(Formatting.GREEN);
            ...
            player.sendMessage(Text.translatable("message.command.heal.self").formatted(Formatting.GREEN));
        }
